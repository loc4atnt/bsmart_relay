...
#define GSM_PIN ""
#define RELAYS_AMOUNT 2

int relays[RELAYS_AMOUNT] = {21,22};
...
// Wifi config
const char* ssid     = "DUC LOC";
const char* password = "02573558170";
// MQTT config
...
void mqttCallback(char* topic, byte* payload, unsigned int len) {
 ...
 // Nếu topic esp nhận được trùng với topic_relays thì bắt đầu xử lý dữ liệu điều khiển relays
...
void mqttLoopWithWifi() {
  ...
  if (!wifiMqtt.connected()) {// Nếu không có kết nối tới broker thì chạy hàm kết nối
    Serial.println("Ket noi MQTT voi Wifi");
    while ( (!wifiMqtt.connected()) && isWifiConnected()) {
      connectMQTT(&wifiMqtt);// Hàm kết nối MQTT client tới MQTT Broker
      delay(5000);
    }
  } else // Nếu đã kết nối tới broker thì chạy hàm xử lý
...
void mqttLoopWithGPRS() {
  ...
  if (!gsmMqtt.connected()) {// Nếu không có kết nối tới broker thì chạy hàm kết nối
    Serial.println("Ket noi MQTT voi GPRS");
    while ( (!gsmMqtt.connected()) && modem.isGprsConnected()) {
      connectMQTT(&gsmMqtt);// Hàm kết nối MQTT client tới MQTT Broker
      delay(5000);
    }
  } else // Nếu đã kết nối tới broker thì chạy hàm xử lý
